with
  monthly_users_by_title as (
    select *
    from {{ ref('salesforce_monthly_users_by_title') }}
  ),

  data_sales_cost_split_monthly as (
    select *
    from {{ ref('data_sales_cost_split_monthly') }}
  ),
  ...
  eae_headcount_ratio_monthly as (
    select date_month
      , sum(user_count) filter(where
        ...
        / sum(user_count) as eae_qc_perc
      , sum(user_count) filter(where
        title = 'Enterprise Account Executive'
        and province <> 'QC'
        )
        / sum(user_count) as eae_roc_perc
      , sum(user_count) filter(where
        title = 'Account Executive'
        and province = 'QC'
        )
        / sum(user_count) as ae_qc_perc
      , sum(user_count) filter(where
        title = 'Account Executive'
        and province <> 'QC'
        )
        / sum(user_count) as ae_roc_perc
