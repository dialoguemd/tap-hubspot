...
with countdown_question_replied as (
  ...
  select qnaire_tid
          ...
          , case when question_name like '%q1' then 'Little interest or pleasure in doing things'
                 ...
                 when question_name like '%q3' then 'Feeling nervous, anxious or on edge'
                 when question_name like '%q4' then 'Not being able to stop or control worrying'
                 when question_name like '%q5' then 'Unable to control the important things in your life'
                 when question_name like '%q6' then 'Confident about your ability to handle your personal problems'
                 when question_name like '%q7' then 'That things were going your way'
                 when question_name like '%q8' then 'Difficulties were piling up so high that you could not overcome them'
              end as question
          , question_category
          , replace(
...
select answers.qnaire_tid
    ...
    , case when max(reply_value) filter(where question_category = 'Depression') >= 2 then true else false end as depression_flag
    , sum(reply_value) filter(where question_category = 'Anxiety') as anxiety_score
