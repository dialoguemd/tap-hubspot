...
class HubSpotBookmarks1(unittest.TestCase):
    ...
    def record_to_bk_value(self, stream, record):
        ...
        if stream == "companies":
            bk_value = record.get("property_hs_lastmodifieddate") or record.get(
                "createdate"
            )
            if bk_value is None:
                return None
        ...
        if stream == "deals":
            ...
            if not bk_value:
                raise Exception(
                    "Record received without bookmark value for stream {}: {}".format(
                        stream, record
                    )
                )
    ...
    def test_run(self):
        ...
        replicated_row_count = reduce(
            lambda accum, c: accum + c, record_count_by_stream.values()
        )
        self.assertGreater(
        ...
        max_bookmarks_from_records = runner.get_most_recent_records_from_target(
            self, self.expected_bookmarks(), self.get_properties()["start_date"]
        )
        ...
        for k in self.expected_bookmarks().keys():
            if max_bookmarks_from_records.get(k) is None:
                max_bookmarks_from_records[k] = utils.strftime(
                    datetime.datetime(2017, 5, 1, 0, 0, 0, 0, datetime.timezone.utc)
                )
        ...
        for k, v in sorted(list(self.expected_bookmarks().items())):
            for w in v:
                ...
                self.assertEqual(
                    ...
                    "Bookmark {} ({}) for stream {} should have been updated to {}".format(
                        bk_value, w, k, max_bookmarks_from_records[k]
                    ),
                )
                print(
                    "bookmark {}({}) updated to {} from max record value {}".format(
                        k, w, bk_value, max_bookmarks_from_records[k]
                    )
                )
        ...
        for k, v in self.expected_offsets().items():
            self.assertEqual(
                ...
                v,
                msg="unexpected offset found for stream {} {}. state: {}".format(
                    k, v, state
                ),
            )
        ...
        self.assertEqual(
            ...
            None,
            "Unexpected `currently_syncing` bookmark value: {} Expected: None".format(
                state.get("currently_syncing")
            ),
        )
